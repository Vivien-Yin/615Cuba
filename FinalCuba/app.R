  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(
        ...
        tabsetPanel(
          ...
          tabPanel(
            ...
            div(
              p("Cuba faces a range of political challenges that impact its domestic stability, international relationships, and economic development. These challenges stem from both internal governance issues and external geopolitical pressures."),
              ...
              tags$div(
                ...
                tags$h4("1. U.S. Economic Embargo"),
                tags$p("One of the most significant external threats to Cuba’s political and economic stability is the long-standing U.S. economic embargo. Implemented in 1960, the embargo restricts Cuba’s ability to trade freely with the United States, which was historically one of its largest trading partners."),
                tags$h4("2. Dependency on Strategic Allies"),
                tags$p("Cuba’s reliance on strategic alliances for economic and political support, particularly with countries like Venezuela, Russia, and China, poses risks such as Economic Vulnerability and Geopolitical Risks. Venezuela’s economic crisis significantly reduced the oil subsidies Cuba had relied on, forcing it to seek alternative energy sources. Aligning with countries that oppose U.S. influence has made Cuba a target of intensified sanctions and diplomatic isolation."),
                tags$h4("3. International Sanctions and Diplomatic Isolation"),
                tags$p("Cuba faces diplomatic isolation in some regions due to its socialist model and alignment with authoritarian regimes."),
                tags$p("European Union Relations: While relations have improved, the EU continues to pressure Cuba for human rights reforms."),
                tags$p("UN Sanctions Pressure: The U.S. frequently lobbies for continued sanctions on Cuba, limiting its ability to fully participate in international trade.")
              )
            )
          )
        )
      ...
      tabItem(
        ...
        p("5. Machín-Gómez, G. (2008). Political Challenges and External Pressure: A Case Study of Cuba. Policy Perspectives, 5(3), 19-29."),
        p("6. Gunn, G. (1991). Cuba in Crisis. Current History, 90(554), 101-135."),
        p("7. Baloyra, E. A., & Morris, J. A. (Eds.). (1993). Conflict and change in Cuba. UNM Press.")
      )
    )
  )
)
...
server <- function(input, output, session) {
  ...
  output$map_global <- renderLeaflet({
    leaflet() %>%
      addProviderTiles("CartoDB.Positron") %>%  # Add light map tiles
      setView(lng = -79.5, lat = 21.5, zoom = 2) %>%  # World view (zoom out)
      addCircleMarkers(
        lng = -80,  # Longitude of Cuba
        lat = 21.5, # Latitude of Cuba
        radius = 8,
        color = "blue",
        fillColor = "blue",
        fillOpacity = 1,
        popup = "Cuba Center"  # Tooltip showing "Cuba Center"
      )
  })
  # Map of Cuba
